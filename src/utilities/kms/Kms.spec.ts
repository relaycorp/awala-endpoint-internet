...
describe('Kms', () => {
  describe('retrieveKeyByRef', () => {
    test('Specified key should be imported as raw', async () => {
      ...
      const originalKeySerialised = await derSerialisePrivateKey(privateKey);

      const retrievedKey = await kms.retrievePrivateKeyByRef(originalKeySerialised);

      const retrievedKeySerialised = await derSerialisePrivateKey(retrievedKey);
  ...
  describe('init', () => {
    test('Global provider should be passed to KMS', async () => {
      ...
      const { privateKey } = await generateKeyPair();
      const refBuffer = await derSerialisePrivateKey(privateKey);
